    ...
    <div id="user_promoter_aduit_list">
      ...
      <main-head>
        <el-form :inline="true" size="medium">
          <el-form-item>
            <el-input placeholder="用户账号/ID/手机号" @keyup.enter.native="search" clearable v-model="screenData.keyword"></el-input>
          </el-form-item>
          <el-form-item>
...
<!--          </el-form-item>-->
      ...
      <main-content>
        ...
        <el-table :data="list" stripe v-loading="loading" style="width: 100%" height="570">
          <el-table-column align="center" width="80" prop="id" label="ID编号"></el-table-column>
          ...
          <el-table-column align="center" prop="adGrade" label="分销等级" width="120px">
            <template slot-scope="scope">
              <div class="tag_grade_pt" v-if="scope.row.adGrade == 0">普通会员</div>
              <div class="tag_grade_hj" v-else-if="scope.row.adGrade == 1">黄金会员</div>
              <div class="tag_grade_hh" v-else-if="scope.row.adGrade == 2">合伙人</div>
              <div class="tag_grade_zz" v-else-if="scope.row.adGrade == 3">至尊合伙人</div>
            </template>
          </el-table-column>
          ...
          <el-table-column label="状态" align="center" min-width="180px">
            <template slot-scope="scope">
              <div v-html="getStatus(scope.row.adStatus)"></div>
            </template>
          </el-table-column>
          <el-table-column label="个人业绩" prop="adProAmount" align="center"></el-table-column>
          <el-table-column label="团队业绩" align="center" prop="teamAmount">
          </el-table-column>
          <el-table-column prop="adAmount" label="佣金钱包" align="center">
          </el-table-column>
          <el-table-column align="center" label="操作" width="150px" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button v-if="scope.row.adStatus != 2" class="mybtn" type="text" size="mini" @click="call(scope.row)">取消</el-button>
              ...
              <el-button v-if="scope.row.adStatus == 2" class="mybtn" type="text" size="mini" @click="regain(scope.row)">恢复</el-button>
            </template>
          </el-table-column>
        </el-table>
      </main-content>
      ...
      getStatus(e){
        ...
        grade(row) {
            ...
            this.$prompt('1.普通会员 2.黄金会员 3.合伙人 4.至尊合伙人', '提示', {
                ...
                .then(({ value }) => {
                    this.$request.post({
                        url: 'admin/userManager/changeShopLevel',
                        params: {
                            id: row.id,
                            grade: value - 1
                        },
        ...
        call(row) {
            this.$confirm('此操作将取消资格, 是否继续?', '提示', {
                ...
                .then(() => {
                    this.$request.post({
                        url: '/admin/userManager/updateAdStatus',
                        params: {
                            id: row.id,
                            adStatus: 2
                        },
        ...
        identification() {
            this.$request.post({
                url: '/admin/userManager/markInvite',
                params: {
                    id: this.form.id,
