PR<template>
    <div id="user_promoter_aduit_list">
        ...
        <el-form :inline="true" size="medium">
            ...
            <el-form-item>
                <el-select v-model="screenData.type" placeholder="类型" @change="search">
        ...
        <el-table :data="list" stripe v-loading="loading" style="width: 100%">
            ...
            <el-table-column align="center" prop="queryType" label="分配类型">
                <template slot-scope="scope">
                    订单
                    <!-- <span v-if="scope.row.queryType == 1">平台</span>
                    <span v-else-if="scope.row.queryType == 2">城市代理</span>
                    <span v-else-if="scope.row.queryType == 3">集体</span> -->
                </template>
            </el-table-column>
            <el-table-column align="center" prop="" label="分配内容">
                <template> 商品交易金额分配 </template>
            </el-table-column>
            ...
            <el-table-column align="center" prop="amount" label="分配金额">
                <template slot-scope="scope">
                    ...
                    <span v-if="scope.row.type == 3" class="lanse"> {{ $common.toThousands(scope.row.amount, true) }}</span>
                    <span v-if="scope.row.type == 4" class="info"> {{ $common.toThousands(scope.row.amount, true) }}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="status" label="状态" align="center">
                <template slot-scope="scope">
                    <span v-if="scope.row.type == 1" class="success">已发放</span>
                    <span v-if="scope.row.type == 2" class="error">支出</span>
                    <span v-if="scope.row.type == 3" class="lanse">冻结中</span>
                    <span v-if="scope.row.type == 4" class="info">已退款</span>
                </template>
            </el-table-column>
