<template>
    <div id="authority_account">
        <main-head>
            <span slot="after">
                <el-button plain size="medium" @click="openAddForm" type="primary" icon="el-icon-plus">添加账号</el-button>
            </span>
        </main-head>
        <main-content>
            <!-- 筛选 -->
                ...
                <el-table-column label="操作" align="center" width="310" class-name="operation">
                    <template slot-scope="scope">
                        <div v-if="scope.row.id != 1">
                            ...
                            <el-button type="warning" plain size="small" @click="freeze(scope.row.id)" v-if="scope.row.status == 1">冻结
                            </el-button>
                            <el-button size="small" plain @click="thaw(scope.row.id)" v-if="scope.row.status == -1">解冻</el-button>
