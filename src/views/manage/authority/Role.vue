<template>
	<div id="authority_role">
        <main-head>
            <el-button slot="after" @click="openAddForm" plain type="primary" size="medium" icon="el-icon-plus">添加角色</el-button>
        </main-head>
        ...
        <main-content>
            ...
            <el-table :data="list" stripe style="width: 100%" v-loading="load">
                ...
                <el-table-column label="操作" align="center" width="310" class-name="operation">
                    <template slot-scope="scope">
                        <div >
                            <el-button type="success" plain size="small" @click="getMenuData(scope.row.id)">分配菜单</el-button>
                            <el-button type="primary" plain size="small" @click="setRoleForm(scope.row)">修改</el-button>
                            <el-button type="danger" plain size="small" @click="del(scope.row.id, scope.row.accountNum)">删除</el-button>
                        </div>
                    </template>
                </el-table-column>
            </el-table>
            <el-pagination
                ...
                :total="total"
                @size-change="search"
                @current-change="getList" class="page">
            </el-pagination>
        </main-content>
		...
		<!-- 修改角色弹框 -->
		<el-dialog center title="添加角色" :visible.sync="updateVisible" :close-on-click-modal="false" width="500px">
			<el-form ref="form" :model="roleForm" label-width="80px" :rules="rules">
				<el-form-item label="角色名称" prop="roleName">
					<el-input v-model="roleForm.roleName" placeholder="请输入角色名称"></el-input>
				</el-form-item>
